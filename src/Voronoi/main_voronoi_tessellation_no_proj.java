 ...
 * @author essam
 */
public class main_voronoi_tessellation_no_proj {
...
//		final long scale = 100000;
		...
		double minlat = Double.parseDouble(args[1]) * scale;
		double minlon = (shift + Double.parseDouble(args[2])) * scale;
		double maxlat = Double.parseDouble(args[3]) * scale;
		double maxlon = (shift + Double.parseDouble(args[4])) * scale;
		String towers_file = args[5];
		...
		for (Map.Entry<Integer, Vertex> entrySet : twrs.entrySet()) {
			...
			Point pnt = new Point(v.getX(), (shift * scale + v.getY()));
			points.add(pnt);
		...
		for (Iterator<Point> iterator = points.iterator(); iterator.hasNext();) {
			...
			for (Iterator<Point> pntsIt = region.iterator(); pntsIt.hasNext();) {
				...
				vor_str += "," + next.x + "," + (next.y-shift * scale) + "\n";
				System.out.println("," + next.x + "," + (next.y-shift * scale));
