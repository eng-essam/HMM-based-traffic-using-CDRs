 ...
 * 
 */
public class main_validate_plot_gps_splitted_trips {
	...
	public static void main(String[] args) {
		...
		List<List<String>> obs_l = new ArrayList<>();
		...
		for (int i = 0; i < gps_data.size(); i++) {
			...
			if (tmp_obs.isEmpty()) {
				tmp_obs.add(rec[3]);
			} else if (Integer.parseInt(tmp_obs.get(tmp_obs.size() - 1)) != t) {
			...
			// if (i < gps_data.size() - 30)
			// tmp_obs.add(gps_data.get(i)[3]);

			// debug ..
			if (tmp_obs.size() == 10) {
				obs_l.add(tmp_obs);
				tmp_obs = new ArrayList<>();
				// i++;
				// tmp_obs.add(gps_data.get(i)[3]);
				// break;
			}
		...
		obs_l.stream().forEach(o -> {
			String[] obs = o.toArray(new String[o.size()]);
