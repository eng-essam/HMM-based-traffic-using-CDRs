...
//        System.out.println("time stamps:\t" + tstops);
    ...
    public static void main(String[] args) throws ParseException {
        ...
        String results = "/home/essam/traffic/stops/with_repeated_obs_May-25/";
...
//        final double threshold = Integer.parseInt(args[1]) * 1000;
         ...
         * Data handling flags.
         */
        final boolean weekends = false;
        final boolean trips_flag = false;
        final boolean repeated_obs_flag = true;
//        final boolean log = true;
        double god_threshold = threshold;
        if (KM) {
            god_threshold = god_threshold / 1000;
        }
        int days_cnt = 0;
        Hashtable<Integer, Vertex> towers = DataHandler.readTowers(towerPath);
        GravityModelStops model = new GravityModelStops();
        Hashtable<String, Hashtable<String, Double>> dists = new Hashtable<>(model.calcDistances(towers, KM));
...
//                days_cnt += obs_stops.size();
                for (Map.Entry<String, Hashtable<Integer, Obs>> entrySet : obs_stops.entrySet()) {
                    ...
                    Hashtable<String, Hashtable<Integer, Obs>> day = new Hashtable<>();
                    day.put(key, value);
                    model = new GravityModelStops();
                    model.handle_zones_Flow(day);
                    model.setDistances(dists);
                    ...
                    String fgod_path = results + "data/fgod." + key + ".day.csv";
                    model.writeFGOD(fgod_path, fgod);
                    Hashtable<Double, ArrayList<Double>> avgGOD = model.avgGOD(fgod);
                    fgod_path = results + "data/avg.fgod." + key + ".day.csv";
