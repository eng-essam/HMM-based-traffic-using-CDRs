...
//                = tbuilder.getUTransitions(tbuilder.getTransitionProb());
        Hashtable<String, Hashtable<String, Double>> emission_probability_old
                = adaptor.getEmissionProb(transition_probability, voronoiNeibors);
...
//        Hashtable<Integer, String> obsTable = adaptData.readObservations(obsPath);
        ...
        for (Map.Entry<Integer, String> entrySet : obsTable.entrySet()) {
            String userObs = entrySet.getValue();
//            System.out.println(Arrays.toString(userObs));
            RunnableVA1 va = new RunnableVA1(ploter,
                    userObs,
                    states,
                    transition_probability,
                    emission_probability,
                    emission_probability_old);
            va.start();
        }
        ploter.display_save();
    }
...
class RunnableVA1 implements Runnable {
    ...
    private Hashtable<String, Hashtable<String, Double>> transition_probability;
    private Hashtable<String, Hashtable<String, Double>> emission_probability;
    private Hashtable<String, Hashtable<String, Double>> emission_probability_old;

    RunnableVA1(Plot ploter,
            String threadName,
            String[] states,
            Hashtable<String, Hashtable<String, Double>> transition_probability,
            Hashtable<String, Hashtable<String, Double>> emission_probability,
            Hashtable<String, Hashtable<String, Double>> emission_probability_old) {
        this.userObs = threadName.split(",");

        this.emission_probability = emission_probability;
        this.emission_probability_old = emission_probability_old;
	...
	public void run() {
        ...
        Object[] ret = Viterbi.forward_viterbi(userObs,
                ...
                transition_probability,
                emission_probability);
        System.out.println("Observations: " + threadName);
        System.out.println("-->" + (String) ret[1]);
        this.ploter.plotPath((String) ret[1]);
